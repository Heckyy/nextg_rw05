<?php
	...
	if(!empty($_POST['proses']) && !empty($_SESSION['id_employee'])){
		...
		if($proses=='new' && $_SESSION['request_new']==1){
			if(!empty($_POST['qty']) && !empty($_POST['item'])){

				$type_of_request=mysqli_real_escape_string($db->query, $_POST['type_of_request']);
				...
				$cek=$db->select('tb_request','tanggal LIKE "%'.$bulan_potong.'%"','urut','DESC');
				...
				if(mysqli_num_rows($cek)>0){
					...
					$bulan = substr($date_asli, 5,2);
					$tahun = substr($date_asli, 0,4);
					...
					$db->insert('tb_request','number_request="'.$number.'",tanggal="'.$date_asli.'",id_cluster="'.$cluster.'",id_position="'.$divisi.'",code_cluster="'.$cq['code_cluster'].'",cluster="'.$cq['cluster'].'",position="'.$pq['position'].'",id_employee="'.$employee.'",employee="'.$e['name'].'",urut="'.$urut.'",note="'.$note.'",type_of_request="'.$type_of_request.'",input_data="'.$_SESSION['id_employee'].'"');

					$db->insert('tb_request_detail','number_request="'.$number.'",id_item="'.$item.'",item="'.$i['item'].'",qty="'.$qty.'",unit="'.$unit.'",note="'.$keterangan.'"');
		...
		}else if($proses=='edit' && $_SESSION['request_edit']==1){
			if(!empty($_POST['number']) && !empty($_POST['qty']) && !empty($_POST['item'])){
				...
				$note=mysqli_real_escape_string($db->query, str_replace('and_symbol', '&', $_POST['note']));
				$keterangan=mysqli_real_escape_string($db->query, str_replace('and_symbol', '&', $_POST['keterangan']));
				$tanggal_edit=$_POST['tanggalEdit'];
				...
				if(mysqli_num_rows($cek)>0){
					...
					if(mysqli_num_rows($detail)>0){
						...
						$db->update('tb_request_detail','qty="'.$qty.'",unit="'.$unit.'",note="'.$keterangan.'"','id_request_detail="'.$d['id_request_detail'].'"');
					}else{
						$db->insert('tb_request_detail','number_request="'.$number.'",id_item="'.$item.'",item="'.$i['item'].'",qty="'.$qty.'",unit="'.$unit.'",note="'.$keterangan.'"');
					}

					$db->update('tb_request','id_cluster="'.$cluster.'",tanggal="'.$tanggal_edit.'",id_position="'.$divisi.'",code_cluster="'.$cq['code_cluster'].'",cluster="'.$cq['cluster'].'",position="'.$pq['position'].'",id_employee="'.$employee.'",employee="'.$e['name'].'",note="'.$note.'",update_data="'.$_SESSION['id_employee'].'"','number_request="'.$number.'"');
		...
		}else if($proses=='cancel' && !empty($_SESSION['number_request']) && $_SESSION['request_cancel']==1){
		...
		}else if($proses=='process' && !empty($_SESSION['number_request']) && $_SESSION['request_process']==1){
		...
		}else if($proses=='diketahui' && !empty($_SESSION['number_request']) && $_SESSION['request_process']==1){
		...
		}else if($proses=='hapus' && $_SESSION['request_edit']==1){
			...
			if($type_of_request==2){
				$type_of_request='type_of_item="3"';
			}else{
				$type_of_request='type_of_item!="3"';
			}

			$item=$db->select('tb_item',$type_of_request,'item','ASC');
