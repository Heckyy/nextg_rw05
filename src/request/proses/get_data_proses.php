<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_request','number_request LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" || cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" || position LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" || code_cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" || employee LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%"','id_request','DESC',$startFrom,$perPage);
