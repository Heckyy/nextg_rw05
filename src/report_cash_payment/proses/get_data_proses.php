<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_cash_receipt_payment','id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && number LIKE "%'.$ubah_pencarian.'%" && type="o" && status="1" || id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && dari LIKE "%'.$ubah_pencarian.'%" && type="o" && status="1" || id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && type_of_payment LIKE "%'.$ubah_pencarian.'%" && type="o" && status="1"','id_cash_receipt_payment','DESC',$startFrom,$perPage);
			...
			if(mysqli_num_rows($data) > 0 ){
				...
				foreach ($data as $key => $v) {
					...
					$rows.='"type_of_payment":"'.$v["type_of_payment"].'",';
					$rows.='"untuk":"'.$v["untuk"].'",';
