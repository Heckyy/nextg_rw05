	...
	function view_data(){
		...
		var pag = $('#pagination').simplePaginator({
			...
			pageChange: function(page) {
				...
				console.log(data);
	            $.ajax({
					url:localStorage.getItem('data_link')+"/src/purchasing/proses/get_data_proses_request.php",
					method:"POST",
					data:data,
					type: 'json',
					cache:false,
					success:function(data){	
				        ...
				        if(response.length) {
				          	...
				          	$.each(response, function(key,trk) {
					            ...
					            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.type_of_request+'</td>';
					            ...
					            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.request+'</td>';
					            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.note+'</td>';
					            html += '<td onclick="detail_item(this);" data-id="'+trk.target+'" align="center"><i class="bi bi-eye"></i> | '+status+'</td>';
						        html += '</tr>';
...
function link_view(e){
	...
	document.location.href=localStorage.getItem('data_link')+"/purchasing/new/"+id;
}
...
function detail_item(e){
	...
	console.log(data);
	$.ajax({
		url:localStorage.getItem('data_link')+"/src/request/proses/get_data_proses.php",
		method:"POST",
		data:data,
		type: 'json',
		cache:false,
