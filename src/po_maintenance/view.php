<?php
	class view_po_maintenance{
		
		function data_view($db,$e,$library_class,$code){

			$view=base64_decode($code);
			$view_po_maintenance=$db->select('tb_purchasing','type_of_purchase="2" && number_purchasing="'.$view.'"','id_purchasing','DESC');

			if(mysqli_num_rows($view_po_maintenance)>0 && $_SESSION['purchasing']==1){
					...
					<div class="row justify-content-between align-items-center line">
						<div class="col-auto">
							<h6 class="app-card-title">
								PO Maintenance <b><?php echo $status; ?></b>
							</h6>
						</div>
						<?php 
							if($v['status']==0){
						...
						<tbody>
							...
							<div class="space_line row">
								<div class="col-lg-12">
									...
									?>
										<a href="<?php echo $e; ?>/po-maintenance/new">
											<button type="button" class="btn btn-sm btn-success btn-custom">New</button>
										</a>
									<?php
										}
										if($v['status']=='0' && $_SESSION['po_maintenance_edit']==1){
									?>
											<a href="<?php echo $e; ?>/po-maintenance/edit/<?php echo $code; ?>">
												<button type="button" class="btn btn-sm btn-warning btn-custom">Edit</button>
											</a>
									<?php
										}
										if($inputan==$sekarang && $v['status']!=='2' && $pembayaran=='0' && $_SESSION['po_maintenance_cancel']==1){
									?>
