...
function view_data(){
	...
	var pag = $('#pagination').simplePaginator({
		...
		pageChange: function(page) {
			...
			console.log(data);
            $.ajax({
				url:localStorage.getItem('data_link')+"/src/rt/proses/get_data_proses.php",
				method:"POST",
				data:data,
				type: 'json',
				cache:false,
				success:function(data){	
			        ...
			        if(response.length) {
			          	...
			          	$.each(response, function(key,trk) {
				            ...
				            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.name_representative+'</td>';
...
function link_view(e){
	...
	document.location.href=localStorage.getItem('data_link')+"/rt/view/"+id;
}
