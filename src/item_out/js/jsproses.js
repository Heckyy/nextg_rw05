	...
	function view_data(){
		...
		var pag = $('#pagination').simplePaginator({
			...
			pageChange: function(page) {
				...
				console.log(data);
	            $.ajax({
					url:localStorage.getItem('data_link')+"/src/item_out/proses/get_data_proses.php",
					method:"POST",
					data:data,
					type: 'json',
					cache:false,
					success:function(data){	
				        ...
				        if(response.length) {
				          	...
				          	$.each(response, function(key,trk) {
					            ...
					            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.from_for+'</td>';
					            html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.type_receipt_out+'</td>';
...
function link_view(e){
	...
	document.location.href=localStorage.getItem('data_link')+"/item-out/view/"+id;
}
