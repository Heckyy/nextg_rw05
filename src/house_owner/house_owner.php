...
class house_owner
{
	function view_house_owner($db, $e, $library_class, $view, $page)
	{
		...
		if ($_SESSION['house_owner'] == 1) {
			...
			$result = $db->select('tb_population', 'code_population LIKE "%' . $ubah_pencarian . '%" && status<3 || name LIKE "%' . $ubah_pencarian . '%" && status<3 || kk LIKE "%' . $ubah_pencarian . '%" && status<3 || ktp LIKE "%' . $ubah_pencarian . '%" && status<3 || ktp LIKE "%' . $ubah_pencarian . '%" && status<3 || cluster LIKE "%' . $ubah_pencarian . '%" && status<3', 'code_population', 'DESC');
...
?>
			<script src="<?php echo $e; ?>/src/house_owner/js/jsproses.js"></script>
			<div class="app-card-header p-3 main-content container-fluid">
				...
				?>
					<div class="col" align="right">
						<a href="<?php echo $e; ?>/house-owner/new" class="btn btn-default btn-sm link-new">
							New
						</a>
					</div>
				<?php
