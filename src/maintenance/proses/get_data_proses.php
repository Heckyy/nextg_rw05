<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_request','number_request LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || position LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || code_cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || employee LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2"','id_request','DESC',$startFrom,$perPage);
