<?php
	class maintenance{
		...
		function view_maintenance($db,$e,$library_class,$view,$page){

			if($_SESSION['maintenance']==1){
				...
				$result = $db->select('tb_request','number_request LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || position LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || code_cluster LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2" || employee LIKE "%'.$ubah_pencarian.'%" && tanggal LIKE "%'.$priod.'%" && type_of_request="2"','id_request','DESC');
...
?>
				...
				<script src="<?php echo $e; ?>/src/maintenance/js/jsproses.js"></script>
				<div class="app-card-header p-3 main-content container-fluid">
					...
					<div class="col" align="right">
						<?php
							if($_SESSION['po_maintenance']==1){
						?>
								<a href="<?php echo $e; ?>/po-maintenance" class="btn btn-primary btn-sm">
						           	PO Maintenance
								</a>
						<?php
							}
						...
						?>
								<a href="<?php echo $e; ?>/maintenance/new" class="btn btn-default btn-sm link-new">
						           	New
								</a>
						<?php
							}
