<?php
	class view_maintenance{
		...
		function data_view($db,$e,$library_class,$code){
			...
			$view_request=$db->select('tb_request','number_request="'.$view.'" && type_of_request="2"','id_request','DESC');

			if(mysqli_num_rows($view_request)>0 && $_SESSION['maintenance']==1){
					...
					<div class="row justify-content-between align-items-center line">
						<div class="col-auto">
							<h6 class="app-card-title">
								Maintenance <b><?php echo $status; ?></b>
							</h6>
						</div>
						<?php 
							if($v['status']==0){
						...
						<div class="space_line row">
							...
							<div class="space_line row">
								<div class="col-lg-12">
									...
									?>
										<a href="<?php echo $e; ?>/maintenance/new">
											<button type="button" class="btn btn-sm btn-success btn-custom">New</button>
										</a>
									<?php
										}
										if($v['status']=='0' && $_SESSION['maintenance_edit']==1){
									?>
											<a href="<?php echo $e; ?>/maintenance/edit/<?php echo $code; ?>">
												<button type="button" class="btn btn-sm btn-warning btn-custom">Edit</button>
											</a>
									<?php
										}
										if($inputan==$sekarang && $v['status']!=='2' && $_SESSION['maintenance_cancel']==1){
											if($jum_purchasing==0){
									?>
