<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_unit','code_unit LIKE "%'.$ubah_pencarian.'%" || unit LIKE "%'.$ubah_pencarian.'%"','code_unit','ASC',$startFrom,$perPage,'id_unit,code_unit,unit,note');
			...
			if(mysqli_num_rows($data) > 0 ){
				...
				foreach ($data as $key => $v) {
					...
					$ubah=str_replace("=", "", base64_encode($v['code_unit']));

					$rows.='{"no":"'.$no.'",';
					$rows.='"target":"'.$ubah.'",';
					$rows.='"code_unit":"'.$v["code_unit"].'",';
					$rows.='"unit":"'.$v["unit"].'",';
					$rows.='"note":"'.$v["note"].'"}'; 
