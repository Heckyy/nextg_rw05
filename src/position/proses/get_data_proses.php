<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_position','code_position LIKE "%'.$ubah_pencarian.'%" || position LIKE "%'.$ubah_pencarian.'%"','code_position','ASC',$startFrom,$perPage,'id_position,code_position,position,note');
			...
			if(mysqli_num_rows($data) > 0 ){
				...
				foreach ($data as $key => $v) {
					...
					$ubah=str_replace("=", "", base64_encode($v['code_position']));

					$rows.='{"no":"'.$no.'",';
					$rows.='"target":"'.$ubah.'",';
					$rows.='"code_position":"'.$v["code_position"].'",';
