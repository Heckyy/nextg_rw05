<?php
	class wh_type_of_receipt{
		
		function view_wh_type_of_receipt($db,$e,$library_class,$view,$page){
...
?>
								$("#account_<?php echo $r['id_type_of_receipt_wh']; ?>").select2({
								    theme: "bootstrap-5",
								});
<?php
						}
					...
					<div class="row justify-content-between align-items-center line">
						<div class="col-auto">
							<h6 class="app-card-title">
								Account - Warehouse Type of Receipt
							</h6>
						</div>
					</div>
					<?php
						    ...
						    <tbody>
						    	<?php
						    		if($totalRecords>0){
						    			$no=1;
										foreach ($result as $key => $r) {
											$account = $db->select('tb_account','id_account="'.$r['id_account'].'"','account','ASC');
								?>
											<tr>
												<td align="center">
													<?php
												...
												<td width="50%">
													<?php
														echo $r['type_of_receipt_wh'];
													?>
												</td>
												<td>
													<select id="account_<?php echo $r['id_type_of_receipt_wh']; ?>" name="account_<?php echo $r['id_type_of_receipt_wh']; ?>" class="form-control square" disabled="disabled">
														<option value="">
															Select
														</option>
														<?php
														...
														?>
																<option value="<?php echo $a['id_account']; ?>" selected>
								...
								?>
									<tr>
										<td colspan="3" align="right" class="bg-white">
											<a href="<?php echo $e; ?>/account/wh/type-of-receipt/edit">
