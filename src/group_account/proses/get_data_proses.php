<?php
	...
	if(!empty($_SESSION['id_employee']) && !empty($_POST['proses'])){
		...
		if($_POST['proses']=='tarik_data'){
			...
			$data = $db->selectpage('tb_group_account','group_account LIKE "%'.$ubah_pencarian.'%" || position LIKE "%'.$ubah_pencarian.'%" || type_of_report LIKE "%'.$ubah_pencarian.'%"','group_account','ASC',$startFrom,$perPage,'id_group_account,group_account,position,type_of_report');
			...
			if(mysqli_num_rows($data) > 0 ){
				...
				foreach ($data as $key => $v) {
					...
					$ubah=str_replace("=", "", base64_encode($v['id_group_account']));
