	...
	function view_data(){
		...
		var pag = $('#pagination').simplePaginator({
			...
			pageChange: function(page) {
				...
				console.log(data);
	            $.ajax({
					url:localStorage.getItem('data_link')+"/src/inv_purchasing/proses/get_data_proses.php",
					method:"POST",
					data:data,
					type: 'json',
					cache:false,
					success:function(data){	
				        ...
				        if(response.length) {
				          	...
				          	$.each(response, function(key,trk) {
					            ...
					            html += '<tr'+bg+'>';
								html += '<td onclick="link_view(this);" data-id="'+trk.target+'">'+trk.no+'</td>';
					            ...
					            html += '<td class = "text-center" onclick="link_view(this);" data-id="'+trk.target+'">'+trk.status+'</td>';
						        html += '</tr>';
...
function link_view(e){
	...
	document.location.href=localStorage.getItem('data_link')+"/inv_purchasing/view/"+id;
}
