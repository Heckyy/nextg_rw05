<?php
	class report_cash_receipt{
		
		function view_report_cash_receipt($db,$e,$library_class,$view,$page){
			...
			if($_SESSION['report_cash_receipt']==1){
				...
				$result = $db->select('tb_cash_receipt_payment','id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && number LIKE "%'.$ubah_pencarian.'%" && type="i" && status="1" || id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && dari LIKE "%'.$ubah_pencarian.'%" && type="i" && status="1" || id_bank="'.$select_bank.'" && tanggal LIKE "%'.$priod.'%" && type_of_receipt LIKE "%'.$ubah_pencarian.'%" && type="i" && status="1"','id_cash_receipt_payment','DESC');
...
?>
				...
				<script src="<?php echo $e; ?>/src/report_cash_receipt/js/jsproses.js"></script>
				<div class="app-card-header p-3 main-content container-fluid">
					<div class="row justify-content-between align-items-center line">
						<div class="col-auto">
							<h6 class="app-card-title">
								Report Cash Receipt <b>( <?php echo $bn['bank_cash']; ?> )</b>
							</h6>
						</div>
					    <div class="col-auto">
